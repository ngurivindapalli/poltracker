...
import { Affiliation } from '@/lib/types/senatorExtended'
...
const AFFILIATION_TYPE_COLORS: Record<string, { bg: string; border: string; text: string }> = {
  ...
  Committee: { bg: '#FCE7F3', border: '#EC4899', text: '#9F1239' },
...
export default function AffiliationsSection({ bioguideId }: AffiliationsSectionProps) {
  ...
  if (error || affiliations.length === 0) {
    return (
      <section style={{ marginTop: '3rem' }}>
        <h2 style={{ fontSize: '1.6rem', marginBottom: '1rem', fontWeight: 600 }}>
          Affiliations & Organizations
        ...
        >
          {error || 'Affiliation data is not currently available. This information combines data from Wikidata, investment analysis, and donor industry ties.'}
        </div>
      </section>
    )
  }
    ...
    <section style={{ marginTop: '3rem' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        {Object.entries(grouped).map(([type, affs]) => {
          const typeColor = AFFILIATION_TYPE_COLORS[type] || AFFILIATION_TYPE_COLORS.Other
