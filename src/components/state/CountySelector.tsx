...
export default function CountySelector({ stateCode, stateName, counties: countiesProp }: CountySelectorProps) {
    ...
    async function fetchLocalData() {
      ...
      try {
        const newsRes = await fetch(
          `/api/localNews?state=${encodeURIComponent(stateName)}&county=${encodeURIComponent(selectedCounty)}`
        );
      ...
      // Fetch elections and events from local data
      try {
        ...
        setElections(electionsModule.getElectionsForCounty(stateName, selectedCounty));
  ...
  return (
    <div className="space-y-8">
      {/* County Selector */}
      <Card className="p-6">
        <h3 className="text-[18px] font-bold text-[#1E3A5F] mb-4">
      ...
      {selectedCounty && !loading && (
        <div className="space-y-8">
          {/* Local Elections */}
          <div>
            <h3 className="text-[20px] font-bold text-[#1E3A5F] mb-4 flex items-center gap-2">
                ...
                {elections.map((election, i) => (
                  <Card key={i} className="p-5">
                    <div className="flex items-start justify-between mb-3">
                      <Badge variant="success">{election.type || "Election"}</Badge>
                      <span className="text-[13px] text-[#64748B]">
                        {formatDate(election.date)}
                      </span>
                    </div>
                    <h4 className="text-[16px] font-bold text-[#1E3A5F] mb-2">
                      {election.title}
                    </h4>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {events.map((event, i) => (
                  <Card key={i} className="p-5">
                    ...
                    <p className="text-[13px] text-[#64748B] mb-2 flex items-center gap-1">
