...
/**
 * GET /api/memory/[userId]/brand-context
...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({
      userId,
      brand_context: memory.brand_context,
    })
  } catch (error: any) {
    console.error('Error fetching brand context:', error)
    return NextResponse.json(
      { error: error?.message || 'Failed to fetch brand context' },
      { status: 500 }
    )
  }
}
...
/**
 * PUT /api/memory/[userId]/brand-context
...
/**
 * DELETE /api/memory/[userId]/brand-context
